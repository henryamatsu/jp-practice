...
;
;
const CORRECTION_SYSTEM_INSTRUCTION = `You are a helpful Japanese language tutor providing feedback on student answers.
...
Be brief and concise. End your response after providing feedback. The system will automatically present the next question to the student, so you should not include any questions in your response.`;
...
REVIEW SYSTEM:
- If this is a review generation (indicated in the prompt), regenerate the EXACT same 5 questions from the previous set
- If this is for new questions, create 5 completely new questions that are different from any previous questions
...
class GeminiEngine {
   ...
   */ getCorrectionSystemInstruction(topic, difficulty, useJapanese) {
        let instruction = CORRECTION_SYSTEM_INSTRUCTION;
   ...
   */ async generateResponse(messages, topic, difficulty, useJapanese, currentQuestion) {
        // If topic is random, pick a random topic for this session
        const actualTopic = topic === "random" ? this.getRandomTopic() : topic;
        const systemInstruction = this.getCorrectionSystemInstruction(actualTopic, difficulty, useJapanese);
        // Convert message history to Gemini format
        const contents = this.convertMessagesToContents(messages);
        // Add context about the current question
